...
public class Driver2 {
  ...
  static class RevertTransaction extends Transaction {
  ...
  public static void main(String[] args) {
    ...
    while (masukan.hasNext()) {
      ...
      if (command.equals("remove-account")) {
        ...
        String accountName = inputArr[1];
        int idRevert = Integer.parseInt(inputArr[2]);
        Date posted_at = null;
        try {
          posted_at = dateFormat.parse(inputArr[3]);
        } catch (ParseException e) {
        ...
        for (Account account : accounts) {
          if (account.getAccountName().toLowerCase().equals(searchKey)) {
            for (Transaction transaction : transactions) {
              if (transaction.getIdTransaction() == idRevert) {
                try {
                  account.BalanceTest(transaction.getAmount() * (-1));
                } catch (Exception e) {
                  if (e.getMessage().equals("Berikan Izin")) {
                    try {
                      account.BalanceTest(transaction.getAmount() * (-1));
                    } catch (Exception f) {
                      if (f.getMessage().equals("Berikan Izin Revert")) {
                        idTransaction++;
                        account.addBalance(transaction.getAmount() * (-1));
                        Transaction revertTransaction = new RevertTransaction(idTransaction, accountName, idRevert,
                            transaction.getAmount() * (-1), posted_at, "REVERT: " + transaction.getNote());
                        transactions.add(revertTransaction);
                        break;
                      }
                    }
                  }
